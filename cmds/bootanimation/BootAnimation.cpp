...
#define TYPE_FORCED 3  // Must match org.derpfest.view.DisplayResolutionManager.TYPE_FORCED
...
status_t BootAnimation::initDisplaysAndSurfaces() {
    ...
    for (size_t displayIdx = 0; displayIdx < mDisplays.size(); displayIdx++) {
        ...
        const bool shouldScale =
                android::base::GetIntProperty("ro.derpfest.display.resolution_switch", 0) == TYPE_FORCED &&
                android::base::GetIntProperty("persist.sys.derpfest.bootanimation.scale", 0) == 1;
        if (shouldScale) {
